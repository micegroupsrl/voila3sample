...
public class ClienteController extends BaseController<Cliente> {
   ...
   *     Cliente, or with status {@code 400 (Bad Request)} if the requestBody is invalid.
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the cliente, or
  ...
  public ResponseEntity<ViewClienteDto> read(@PathVariable String objectKey) {
    Optional<Cliente> opt =
        Optional.of(
            clienteService
                .findByObjectKey(objectKey)
                .orElseThrow(
                    () -> new ResourceNotFoundException(Cliente.class.getSimpleName(), objectKey)));
   ...
   *     Cliente, or with status {@code 400 (Bad Request)} if the requestBody is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the Cliente couldn't be updated.
   */
  @PutMapping
  ...
  /**
   * {@code DELETE /cliente/:objectKey} : Delete the cliente with given objectKey.
   *
  ...
  /**
   * {@code GET /cliente/search?filter=:query} : Get the cliente filtered by given query.
   *
   * @param query the query to execute filtering the Cliente to retrieve.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the cliente.
   */
  @GetMapping("/search")
  @Operation(summary = "Get the Cliente filtered by given query")
  @PreAuthorize("hasRole(@permissionHolder.CLIENTE_SEARCH.toString())")
  public ResponseEntity<Page<ViewClienteDto>> search(
      @Filter Specification<Cliente> specification, Pageable page) {
  ...
  // Xls Report
  /**
   * GET: /print/xlsx-list: Return the XLS list report for Cliente filtered by specification in
   * input GET.
   *
   * @param criteria
  ...
  public void printXlsxList(
      @Filter Specification<Cliente> specification, HttpServletResponse response) {
  ...
  /**
   * Returns a ResponseEntity containing a ViewClienteDto mapped by the Optional<Cliente>, header
   * and status passed in input
   */
  private ResponseEntity<ViewClienteDto> toResponseEntity(
      Optional<Cliente> maybeResponse, HttpHeaders header, HttpStatus status) {
    return maybeResponse
        .map(response -> new ResponseEntity<>(clienteMapper.map(response), header, status))
