...
public class ProdottoController extends BaseController<Prodotto> {
   ...
   *     Prodotto, or with status {@code 400 (Bad Request)} if the requestBody is invalid.
  ...
  public ResponseEntity<ViewProdottoDto> insert(@RequestBody @Valid EditProdottoDto requestBody) {
    ...
    if (prodottoService.findByObjectKey(entity.getObjectKey()).isPresent()) {
      throw new ResourceAlreadyFoundException(
          Prodotto.class.getSimpleName(), entity.getObjectKey());
    } else {
      ...
      URI location =
          ServletUriComponentsBuilder.fromCurrentRequest()
              .path("/{id}")
              .buildAndExpand(entity.getIdProdotto())
              .toUri();
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the prodotto, or
  ...
  public ResponseEntity<ViewProdottoDto> read(@PathVariable String objectKey) {
    Optional<Prodotto> opt =
        Optional.of(
            prodottoService
                .findByObjectKey(objectKey)
                .orElseThrow(
                    () ->
                        new ResourceNotFoundException(Prodotto.class.getSimpleName(), objectKey)));
   ...
   *     Prodotto, or with status {@code 400 (Bad Request)} if the requestBody is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the Prodotto couldn't be updated.
   */
  @PutMapping
  ...
  /**
   * {@code DELETE /prodotto/:objectKey} : Delete the prodotto with given objectKey.
   *
  ...
  /**
   * {@code GET /prodotto/search?filter=:query} : Get the prodotto filtered by given query.
   *
   * @param query the query to execute filtering the Prodotto to retrieve.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the prodotto.
   */
  @GetMapping("/search")
  @Operation(summary = "Get the Prodotto filtered by given query")
  @PreAuthorize("hasRole(@permissionHolder.PRODOTTO_SEARCH.toString())")
  public ResponseEntity<Page<ViewProdottoDto>> search(
      @Filter Specification<Prodotto> specification, Pageable page) {
    Page<ViewProdottoDto> collModel =
        prodottoMapper.map(prodottoService.search(specification, page));
    return toResponseEntityPaged(collModel, null);
  }
  ...
  // Xls Report
  /**
   * GET: /print/xlsx-list: Return the XLS list report for Prodotto filtered by specification in
   * input GET.
   *
   * @param criteria
  ...
  public void printXlsxList(
      @Filter Specification<Prodotto> specification, HttpServletResponse response) {
    byte[] body = prodottoService.printXLSList(specification);
    prepareReportResponse(
        "Elenco Prodotto.xlsx",
        body,
        response,
  ...
  /**
   * Returns a ResponseEntity containing a ViewProdottoDto mapped by the Optional<Prodotto>, header
   * and status passed in input
   */
  private ResponseEntity<ViewProdottoDto> toResponseEntity(
      Optional<Prodotto> maybeResponse, HttpHeaders header, HttpStatus status) {
    return maybeResponse
        .map(response -> new ResponseEntity<>(prodottoMapper.map(response), header, status))
