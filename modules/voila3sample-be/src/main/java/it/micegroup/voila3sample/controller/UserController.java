...
public class UserController extends BaseController<User> {
	...
	public ResponseEntity<ViewUserDto> insert(@RequestBody @Valid EditUserDto requestBody) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(entity.getUserId())
				.toUri();
	...
	public ResponseEntity<ViewUserDto> read(@PathVariable String objectKey) {
		Optional<User> opt = Optional.of(userService.findByObjectKey(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(User.class.getSimpleName(), objectKey)));
	...
	public ResponseEntity<ViewUserDto> delete(@PathVariable String objectKey) {
		Optional<User> opt = Optional.of(userService.delete(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(User.class.getSimpleName(), objectKey)));
		return toResponseEntity(opt, null, HttpStatus.OK);
	}
	...
	@PreAuthorize("hasRole(@permissionHolder.USER_SEARCH.toString())")
	public ResponseEntity<Page<ViewUserDto>> search(@Filter Specification<User> specification, Pageable page) {
	...
	// Xls Report
	/**
	 * GET: /print/xlsx-list: Return the XLS list report for User filtered by
	...
	@PreAuthorize("hasRole(@permissionHolder.USER_REPORT.toString())")
	public void printXlsxList(@Filter Specification<User> specification, HttpServletResponse response) {
		byte[] body = userService.printXLSList(specification);
		prepareReportResponse("Elenco User.xlsx", body, response, MediaType.parseMediaType("application/vnd.ms-excel"),
				"application/octet-stream");
	}

	/**
	 * Returns a ResponseEntity containing a ViewUserDto mapped by the
	 * Optional<User>, header and status passed in input
	 */
	private ResponseEntity<ViewUserDto> toResponseEntity(Optional<User> maybeResponse, HttpHeaders header,
			HttpStatus status) {
		return maybeResponse.map(response -> new ResponseEntity<>(userMapper.map(response), header, status))
