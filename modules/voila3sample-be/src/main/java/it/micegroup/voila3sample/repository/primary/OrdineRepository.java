...
public interface OrdineRepository extends BaseRepository<Ordine, Integer> {
  ...
  /** Finds all Ordine following the specification in input and return them in a Page */
  ...
  /** Finds all Ordine that are children of the TheStatoOrdine in input and return them in a Page */
  @EntityGraph(value = Ordine.DEFAULT_ENTITY_GRAPH)
  Page<Ordine> findByTheStatoOrdine(StatoOrdine parentEntity, Pageable pageable);

  /** Finds all Ordine that are children of the TheTipoOrdine in input and return them in a Page */
  @EntityGraph(value = Ordine.DEFAULT_ENTITY_GRAPH)
  Page<Ordine> findByTheTipoOrdine(TipoOrdine parentEntity, Pageable pageable);

  /** Finds all Ordine that are children of the TheCliente in input and return them in a Page */
  @EntityGraph(value = Ordine.DEFAULT_ENTITY_GRAPH)
  Page<Ordine> findByTheCliente(Cliente parentEntity, Pageable pageable);

  /**
   * Finds all Ordine that are children of the TheOrdineAggregato in input and return them in a Page
   */
  @EntityGraph(value = Ordine.DEFAULT_ENTITY_GRAPH)
