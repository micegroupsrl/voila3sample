...
public class PrivilegeController extends BaseController<Privilege> {
	 ...
	 *         body the new Privilege, or with status {@code 400 (Bad Request)} if
	...
	public ResponseEntity<ViewPrivilegeDto> insert(@RequestBody @Valid EditPrivilegeDto requestBody) {
		...
		if (privilegeService.findByObjectKey(entity.getObjectKey()).isPresent()) {
			throw new ResourceAlreadyFoundException(Privilege.class.getSimpleName(), entity.getObjectKey());
		} else {

			entity = privilegeService.insert(entity);

			URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
					.buildAndExpand(entity.getPrivilegeId()).toUri();
	 ...
	 *         the privilege, or with status {@code 404 (Not Found)}.
	 */
	...
	public ResponseEntity<ViewPrivilegeDto> read(@PathVariable String objectKey) {
		Optional<Privilege> opt = Optional.of(privilegeService.findByObjectKey(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(Privilege.class.getSimpleName(), objectKey)));
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated Privilege, or with status {@code 400 (Bad Request)} if
	 *         the requestBody is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the Privilege couldn't be
	 *         updated.
	 */
	@PutMapping
	...
	/**
	 * {@code DELETE  /privilege/:objectKey} : Delete the privilege with given
	 * objectKey.
	 *
	...
	public ResponseEntity<ViewPrivilegeDto> delete(@PathVariable String objectKey) {
		Optional<Privilege> opt = Optional.of(privilegeService.delete(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(Privilege.class.getSimpleName(), objectKey)));
		return toResponseEntity(opt, null, HttpStatus.OK);
	}

	/**
	 * {@code GET  /privilege/search?filter=:query} : Get the privilege filtered by
	 * given query.
	 *
	...
	@PreAuthorize("hasRole(@permissionHolder.PRIVILEGE_SEARCH.toString())")
	public ResponseEntity<Page<ViewPrivilegeDto>> search(@Filter Specification<Privilege> specification,
			Pageable page) {
		Page<ViewPrivilegeDto> collModel = privilegeMapper.map(privilegeService.search(specification, page));
		return toResponseEntityPaged(collModel, null);
	}
	...
	// Xls Report
	/**
	 * GET: /print/xlsx-list: Return the XLS list report for Privilege filtered by
	...
	@PreAuthorize("hasRole(@permissionHolder.PRIVILEGE_REPORT.toString())")
	public void printXlsxList(@Filter Specification<Privilege> specification, HttpServletResponse response) {
		byte[] body = privilegeService.printXLSList(specification);
		prepareReportResponse("Elenco Privilege.xlsx", body, response,
				MediaType.parseMediaType("application/vnd.ms-excel"), "application/octet-stream");
	}

	/**
	 * Returns a ResponseEntity containing a ViewPrivilegeDto mapped by the
	 * Optional<Privilege>, header and status passed in input
	 */
	private ResponseEntity<ViewPrivilegeDto> toResponseEntity(Optional<Privilege> maybeResponse, HttpHeaders header,
			HttpStatus status) {
		return maybeResponse.map(response -> new ResponseEntity<>(privilegeMapper.map(response), header, status))
