...
public class PrivilegePerRoleController extends BaseController<PrivilegePerRole> {
	...
	public ResponseEntity<Page<ViewPrivilegePerRoleDto>> findAll(Pageable pageable) {
		Page<ViewPrivilegePerRoleDto> collModel = privilegePerRoleMapper.map(privilegePerRoleService.findAll(pageable));
	 ...
	 *         body the new PrivilegePerRole, or with status
	 *         {@code 400 (Bad Request)} if the requestBody is invalid.
	...
	public ResponseEntity<ViewPrivilegePerRoleDto> insert(@RequestBody @Valid EditPrivilegePerRoleDto requestBody) {
		...
		if (privilegePerRoleService.findByObjectKey(entity.getObjectKey()).isPresent()) {
			throw new ResourceAlreadyFoundException(PrivilegePerRole.class.getSimpleName(), entity.getObjectKey());
		} else {

			entity = privilegePerRoleService.insert(entity);

			URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
					.buildAndExpand(entity.getThePrivilegePerRoleKey()).toUri();
	...
	/**
	 * {@code GET  /privilege-per-role/:objectKey} : Get the privilege-per-role with
	 * given objectKey.
	 *
	 * @param objectKey the objectKey of the privilege-per-role to retrieve.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the privilege-per-role, or with status {@code 404 (Not Found)}.
	 */
	...
	public ResponseEntity<ViewPrivilegePerRoleDto> read(@PathVariable String objectKey) {
		Optional<PrivilegePerRole> opt = Optional.of(privilegePerRoleService.findByObjectKey(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(PrivilegePerRole.class.getSimpleName(), objectKey)));
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated PrivilegePerRole, or with status
	 *         {@code 400 (Bad Request)} if the requestBody is not valid, or with
	 *         status {@code 500 (Internal Server Error)} if the PrivilegePerRole
	 *         couldn't be updated.
	 */
	@PutMapping
	...
	public ResponseEntity<ViewPrivilegePerRoleDto> update(@RequestBody @Valid EditPrivilegePerRoleDto requestBody) {
		PrivilegePerRole entity = privilegePerRoleMapper.map(requestBody);
	...
	/**
	 * {@code DELETE  /privilege-per-role/:objectKey} : Delete the
	 * privilege-per-role with given objectKey.
	 *
	...
	public ResponseEntity<ViewPrivilegePerRoleDto> delete(@PathVariable String objectKey) {
		Optional<PrivilegePerRole> opt = Optional.of(privilegePerRoleService.delete(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(PrivilegePerRole.class.getSimpleName(), objectKey)));
		return toResponseEntity(opt, null, HttpStatus.OK);
	}

	/**
	 * {@code GET  /privilege-per-role/search?filter=:query} : Get the
	 * privilege-per-role filtered by given query.
	 *
	 * @param query the query to execute filtering the PrivilegePerRole to retrieve.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the privilege-per-role.
	 */
	@GetMapping("/search")
	@Operation(summary = "Get the PrivilegePerRole filtered by given query")
	@PreAuthorize("hasRole(@permissionHolder.PRIVILEGE_PER_ROLE_SEARCH.toString())")
	public ResponseEntity<Page<ViewPrivilegePerRoleDto>> search(@Filter Specification<PrivilegePerRole> specification,
			Pageable page) {
		Page<ViewPrivilegePerRoleDto> collModel = privilegePerRoleMapper
				.map(privilegePerRoleService.search(specification, page));
		return toResponseEntityPaged(collModel, null);
	}
	...
	// Xls Report
	/**
	 * GET: /print/xlsx-list: Return the XLS list report for PrivilegePerRole
	 * filtered by specification in input GET.
	 * 
	 * @param criteria
	...
	@Operation(summary = "Return the XLS list report for PrivilegePerRole filtered by criteria in input GET")
	@PreAuthorize("hasRole(@permissionHolder.PRIVILEGE_PER_ROLE_REPORT.toString())")
	public void printXlsxList(@Filter Specification<PrivilegePerRole> specification, HttpServletResponse response) {
		byte[] body = privilegePerRoleService.printXLSList(specification);
		prepareReportResponse("Elenco PrivilegePerRole.xlsx", body, response,
	 ...
	 */
	private ResponseEntity<ViewPrivilegePerRoleDto> toResponseEntity(Optional<PrivilegePerRole> maybeResponse,
			HttpHeaders header, HttpStatus status) {
		return maybeResponse.map(response -> new ResponseEntity<>(privilegePerRoleMapper.map(response), header, status))
