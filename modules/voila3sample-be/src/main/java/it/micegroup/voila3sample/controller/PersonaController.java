...
public class PersonaController extends BaseController<Persona> {
   ...
   *     Persona, or with status {@code 400 (Bad Request)} if the requestBody is invalid.
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the persona, or
  ...
  public ResponseEntity<ViewPersonaDto> read(@PathVariable String objectKey) {
    Optional<Persona> opt =
        Optional.of(
            personaService
                .findByObjectKey(objectKey)
                .orElseThrow(
                    () -> new ResourceNotFoundException(Persona.class.getSimpleName(), objectKey)));
   ...
   *     Persona, or with status {@code 400 (Bad Request)} if the requestBody is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the Persona couldn't be updated.
   */
  @PutMapping
  ...
  /**
   * {@code DELETE /persona/:objectKey} : Delete the persona with given objectKey.
   *
  ...
  /**
   * {@code GET /persona/search?filter=:query} : Get the persona filtered by given query.
   *
   * @param query the query to execute filtering the Persona to retrieve.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the persona.
   */
  @GetMapping("/search")
  @Operation(summary = "Get the Persona filtered by given query")
  @PreAuthorize("hasRole(@permissionHolder.PERSONA_SEARCH.toString())")
  public ResponseEntity<Page<ViewPersonaDto>> search(
      @Filter Specification<Persona> specification, Pageable page) {
  ...
  // Xls Report
  /**
   * GET: /print/xlsx-list: Return the XLS list report for Persona filtered by specification in
   * input GET.
   *
   * @param criteria
  ...
  public void printXlsxList(
      @Filter Specification<Persona> specification, HttpServletResponse response) {
  ...
  /**
   * Returns a ResponseEntity containing a ViewPersonaDto mapped by the Optional<Persona>, header
   * and status passed in input
   */
  private ResponseEntity<ViewPersonaDto> toResponseEntity(
      Optional<Persona> maybeResponse, HttpHeaders header, HttpStatus status) {
    return maybeResponse
        .map(response -> new ResponseEntity<>(personaMapper.map(response), header, status))
