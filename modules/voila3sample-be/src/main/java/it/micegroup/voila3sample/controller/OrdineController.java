...
public class OrdineController extends BaseController<Ordine> {
	...
	public ResponseEntity<ViewOrdineDto> insert(@RequestBody @Valid EditOrdineDto requestBody) {
		...
		if (ordineService.findByObjectKey(entity.getObjectKey()).isPresent()) {
			...
			URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
					.buildAndExpand(entity.getIdOrdine()).toUri();
	...
	// Xls Report
	/**
	 * GET: /print/xlsx-list: Return the XLS list report for Ordine filtered by
	...
	@PreAuthorize("hasRole(@permissionHolder.ORDINE_REPORT.toString())")
	public void printXlsxList(@Filter Specification<Ordine> specification, HttpServletResponse response) {
		byte[] body = ordineService.printXLSList(specification);
		prepareReportResponse("Elenco Ordine.xlsx", body, response,
				MediaType.parseMediaType("application/vnd.ms-excel"), "application/octet-stream");
	}

	/**
	 * Returns a ResponseEntity containing a ViewOrdineDto mapped by the
	 * Optional<Ordine>, header and status passed in input
	 */
	private ResponseEntity<ViewOrdineDto> toResponseEntity(Optional<Ordine> maybeResponse, HttpHeaders header,
			HttpStatus status) {
		return maybeResponse.map(response -> new ResponseEntity<>(ordineMapper.map(response), header, status))
