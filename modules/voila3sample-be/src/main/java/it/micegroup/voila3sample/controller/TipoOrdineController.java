...
public class TipoOrdineController extends BaseController<TipoOrdine> {
   ...
   *     TipoOrdine, or with status {@code 400 (Bad Request)} if the requestBody is invalid.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping
  @Operation(summary = "Create a new TipoOrdine")
  @PreAuthorize("hasRole(@permissionHolder.TIPO_ORDINE_CREATE.toString())")
  public ResponseEntity<ViewTipoOrdineDto> insert(
      @RequestBody @Valid EditTipoOrdineDto requestBody) {
    ...
    if (tipoOrdineService.findByObjectKey(entity.getObjectKey()).isPresent()) {
      throw new ResourceAlreadyFoundException(
          TipoOrdine.class.getSimpleName(), entity.getObjectKey());
    } else {
      ...
      URI location =
          ServletUriComponentsBuilder.fromCurrentRequest()
              .path("/{id}")
              .buildAndExpand(entity.getTheTipoOrdineKey())
              .toUri();
  ...
  /**
   * {@code GET /tipo-ordine/:objectKey} : Get the tipo-ordine with given objectKey.
   *
   * @param objectKey the objectKey of the tipo-ordine to retrieve.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the tipo-ordine,
                ...
                .orElseThrow(
                    () ->
                        new ResourceNotFoundException(
                            TipoOrdine.class.getSimpleName(), objectKey)));
   ...
   *     TipoOrdine, or with status {@code 400 (Bad Request)} if the requestBody is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the TipoOrdine couldn't be updated.
   */
  @PutMapping
  @Operation(summary = "Update an existing TipoOrdine")
  @PreAuthorize("hasRole(@permissionHolder.TIPO_ORDINE_UPDATE.toString())")
  public ResponseEntity<ViewTipoOrdineDto> update(
      @RequestBody @Valid EditTipoOrdineDto requestBody) {
    TipoOrdine entity = tipoOrdineMapper.map(requestBody);
  ...
  /**
   * {@code DELETE /tipo-ordine/:objectKey} : Delete the tipo-ordine with given objectKey.
   *
  ...
  /**
   * {@code GET /tipo-ordine/search?filter=:query} : Get the tipo-ordine filtered by given query.
   *
  ...
  @PreAuthorize("hasRole(@permissionHolder.TIPO_ORDINE_SEARCH.toString())")
  public ResponseEntity<Page<ViewTipoOrdineDto>> search(
      @Filter Specification<TipoOrdine> specification, Pageable page) {
    Page<ViewTipoOrdineDto> collModel =
        tipoOrdineMapper.map(tipoOrdineService.search(specification, page));
    return toResponseEntityPaged(collModel, null);
  }
  ...
  // Xls Report
  /**
   * GET: /print/xlsx-list: Return the XLS list report for TipoOrdine filtered by specification in
   * input GET.
   *
   * @param criteria
  ...
  @Operation(
      summary = "Return the XLS list report for TipoOrdine filtered by criteria in input GET")
  @PreAuthorize("hasRole(@permissionHolder.TIPO_ORDINE_REPORT.toString())")
  public void printXlsxList(
      @Filter Specification<TipoOrdine> specification, HttpServletResponse response) {
    byte[] body = tipoOrdineService.printXLSList(specification);
    prepareReportResponse(
        "Elenco TipoOrdine.xlsx",
        body,
        response,
  ...
  /**
   * Returns a ResponseEntity containing a ViewTipoOrdineDto mapped by the Optional<TipoOrdine>,
   * header and status passed in input
   */
  private ResponseEntity<ViewTipoOrdineDto> toResponseEntity(
      Optional<TipoOrdine> maybeResponse, HttpHeaders header, HttpStatus status) {
    return maybeResponse
        .map(response -> new ResponseEntity<>(tipoOrdineMapper.map(response), header, status))
