...
@NamedEntityGraph(name = RigaOrdine.DEFAULT_ENTITY_GRAPH, attributeNodes = {
		@NamedAttributeNode(RigaOrdine_.THE_PRODOTTO), @NamedAttributeNode(RigaOrdine_.THE_ORDINE) })
public class RigaOrdine extends BaseEntity implements Serializable {
	...
	@EmbeddedId
	private RigaOrdineKey theRigaOrdineKey = new RigaOrdineKey();

	// ATTRIBUTES
	/**
	 * Attribute quantita
	 */
	@Column(name = "quantita", columnDefinition = "NUMERIC(12,2)")
	...
	@ToString.Exclude
	@JoinColumn(name = "id_prodotto", referencedColumnName = "id_prodotto", nullable = false, insertable = false, updatable = false)
	@MapsId("idProdotto")
	...
	@ManyToOne(fetch = FetchType.LAZY)
	@ToString.Exclude
	@JoinColumn(name = "id_ordine", referencedColumnName = "id_ordine", nullable = false, insertable = false, updatable = false)
	@MapsId("idOrdine")
