...
public class RoleController extends BaseController<Role> {
   ...
   *     Role, or with status {@code 400 (Bad Request)} if the requestBody is invalid.
    ...
    if (roleService.findByObjectKey(entity.getObjectKey()).isPresent()) {
      ...
      URI location =
          ServletUriComponentsBuilder.fromCurrentRequest()
              .path("/{id}")
              .buildAndExpand(entity.getRoleId())
              .toUri();
    ...
    Optional<Role> opt =
        Optional.of(
            roleService
                ...
                .orElseThrow(
                    () -> new ResourceNotFoundException(Role.class.getSimpleName(), objectKey)));
  ...
  /**
   * {@code GET /role/search?filter=:query} : Get the role filtered by given query.
   *
  ...
  @PreAuthorize("hasRole(@permissionHolder.ROLE_SEARCH.toString())")
  public ResponseEntity<Page<ViewRoleDto>> search(
      @Filter Specification<Role> specification, Pageable page) {
  ...
  // Xls Report
  /**
   * GET: /print/xlsx-list: Return the XLS list report for Role filtered by specification in input
   * GET.
   *
   * @param criteria
  ...
  public void printXlsxList(
      @Filter Specification<Role> specification, HttpServletResponse response) {
  ...
  /**
   * Returns a ResponseEntity containing a ViewRoleDto mapped by the Optional<Role>, header and
   * status passed in input
   */
  private ResponseEntity<ViewRoleDto> toResponseEntity(
      Optional<Role> maybeResponse, HttpHeaders header, HttpStatus status) {
    return maybeResponse
        .map(response -> new ResponseEntity<>(roleMapper.map(response), header, status))
