...
public class CategoriaOrdineController extends BaseController<CategoriaOrdine> {
	...
	public ResponseEntity<Page<ViewCategoriaOrdineDto>> findAll(Pageable pageable) {
		Page<ViewCategoriaOrdineDto> collModel = categoriaOrdineMapper.map(categoriaOrdineService.findAll(pageable));
	 ...
	 *         body the new CategoriaOrdine, or with status
	...
	public ResponseEntity<ViewCategoriaOrdineDto> insert(@RequestBody @Valid EditCategoriaOrdineDto requestBody) {
		...
		if (categoriaOrdineService.findByObjectKey(entity.getObjectKey()).isPresent()) {
			throw new ResourceAlreadyFoundException(CategoriaOrdine.class.getSimpleName(), entity.getObjectKey());
		} else {

			entity = categoriaOrdineService.insert(entity);

			URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
					.buildAndExpand(entity.getIdCategoriaOrdine()).toUri();
	...
	/**
	 * {@code GET  /categoria-ordine/:objectKey} : Get the categoria-ordine with
	 * given objectKey.
	 *
	 * @param objectKey the objectKey of the categoria-ordine to retrieve.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the categoria-ordine, or with status {@code 404 (Not Found)}.
	 */
	...
	public ResponseEntity<ViewCategoriaOrdineDto> read(@PathVariable String objectKey) {
		Optional<CategoriaOrdine> opt = Optional.of(categoriaOrdineService.findByObjectKey(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(CategoriaOrdine.class.getSimpleName(), objectKey)));
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated CategoriaOrdine, or with status {@code 400 (Bad Request)}
	 *         if the requestBody is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the CategoriaOrdine couldn't
	 *         be updated.
	 */
	@PutMapping
	...
	public ResponseEntity<ViewCategoriaOrdineDto> update(@RequestBody @Valid EditCategoriaOrdineDto requestBody) {
		CategoriaOrdine entity = categoriaOrdineMapper.map(requestBody);
	...
	/**
	 * {@code DELETE  /categoria-ordine/:objectKey} : Delete the categoria-ordine
	 * with given objectKey.
	 *
	...
	public ResponseEntity<ViewCategoriaOrdineDto> delete(@PathVariable String objectKey) {
		Optional<CategoriaOrdine> opt = Optional.of(categoriaOrdineService.delete(objectKey)
				.orElseThrow(() -> new ResourceNotFoundException(CategoriaOrdine.class.getSimpleName(), objectKey)));
		return toResponseEntity(opt, null, HttpStatus.OK);
	}

	/**
	 * {@code GET  /categoria-ordine/search?filter=:query} : Get the
	 * categoria-ordine filtered by given query.
	 *
	 * @param query the query to execute filtering the CategoriaOrdine to retrieve.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the categoria-ordine.
	 */
	@GetMapping("/search")
	@Operation(summary = "Get the CategoriaOrdine filtered by given query")
	@PreAuthorize("hasRole(@permissionHolder.CATEGORIA_ORDINE_SEARCH.toString())")
	public ResponseEntity<Page<ViewCategoriaOrdineDto>> search(@Filter Specification<CategoriaOrdine> specification,
			Pageable page) {
		Page<ViewCategoriaOrdineDto> collModel = categoriaOrdineMapper
				.map(categoriaOrdineService.search(specification, page));
		return toResponseEntityPaged(collModel, null);
	}
	...
	// Xls Report
	/**
	 * GET: /print/xlsx-list: Return the XLS list report for CategoriaOrdine
	...
	@Operation(summary = "Return the XLS list report for CategoriaOrdine filtered by criteria in input GET")
	@PreAuthorize("hasRole(@permissionHolder.CATEGORIA_ORDINE_REPORT.toString())")
	public void printXlsxList(@Filter Specification<CategoriaOrdine> specification, HttpServletResponse response) {
		byte[] body = categoriaOrdineService.printXLSList(specification);
		prepareReportResponse("Elenco CategoriaOrdine.xlsx", body, response,
	 ...
	 */
	private ResponseEntity<ViewCategoriaOrdineDto> toResponseEntity(Optional<CategoriaOrdine> maybeResponse,
			HttpHeaders header, HttpStatus status) {
		return maybeResponse.map(response -> new ResponseEntity<>(categoriaOrdineMapper.map(response), header, status))
