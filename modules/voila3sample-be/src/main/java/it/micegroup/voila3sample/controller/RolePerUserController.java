...
public class RolePerUserController extends BaseController<RolePerUser> {
  ...
  public ResponseEntity<Page<ViewRolePerUserDto>> findAll(Pageable pageable) {
    Page<ViewRolePerUserDto> collModel =
        rolePerUserMapper.map(rolePerUserService.findAll(pageable));
   ...
   *     RolePerUser, or with status {@code 400 (Bad Request)} if the requestBody is invalid.
  ...
  public ResponseEntity<ViewRolePerUserDto> insert(
      @RequestBody @Valid EditRolePerUserDto requestBody) {
    ...
    if (rolePerUserService.findByObjectKey(entity.getObjectKey()).isPresent()) {
      throw new ResourceAlreadyFoundException(
          RolePerUser.class.getSimpleName(), entity.getObjectKey());
    } else {
      ...
      URI location =
          ServletUriComponentsBuilder.fromCurrentRequest()
              .path("/{id}")
              .buildAndExpand(entity.getTheRolePerUserKey())
              .toUri();
  ...
  /**
   * {@code GET /role-per-user/:objectKey} : Get the role-per-user with given objectKey.
   *
   * @param objectKey the objectKey of the role-per-user to retrieve.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the
   *     role-per-user, or with status {@code 404 (Not Found)}.
   */
                ...
                .orElseThrow(
                    () ->
                        new ResourceNotFoundException(
                            RolePerUser.class.getSimpleName(), objectKey)));
   ...
   *     RolePerUser, or with status {@code 400 (Bad Request)} if the requestBody is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the RolePerUser couldn't be updated.
   */
  @PutMapping
  @Operation(summary = "Update an existing RolePerUser")
  @PreAuthorize("hasRole(@permissionHolder.ROLE_PER_USER_UPDATE.toString())")
  public ResponseEntity<ViewRolePerUserDto> update(
      @RequestBody @Valid EditRolePerUserDto requestBody) {
    RolePerUser entity = rolePerUserMapper.map(requestBody);
  ...
  /**
   * {@code DELETE /role-per-user/:objectKey} : Delete the role-per-user with given objectKey.
   *
  ...
  /**
   * {@code GET /role-per-user/search?filter=:query} : Get the role-per-user filtered by given
   * query.
   *
  ...
  @PreAuthorize("hasRole(@permissionHolder.ROLE_PER_USER_SEARCH.toString())")
  public ResponseEntity<Page<ViewRolePerUserDto>> search(
      @Filter Specification<RolePerUser> specification, Pageable page) {
    Page<ViewRolePerUserDto> collModel =
        rolePerUserMapper.map(rolePerUserService.search(specification, page));
    return toResponseEntityPaged(collModel, null);
  }
  ...
  // Xls Report
  /**
   * GET: /print/xlsx-list: Return the XLS list report for RolePerUser filtered by specification in
   * input GET.
   *
   * @param criteria
  ...
  @Operation(
      summary = "Return the XLS list report for RolePerUser filtered by criteria in input GET")
  @PreAuthorize("hasRole(@permissionHolder.ROLE_PER_USER_REPORT.toString())")
  public void printXlsxList(
      @Filter Specification<RolePerUser> specification, HttpServletResponse response) {
    byte[] body = rolePerUserService.printXLSList(specification);
    prepareReportResponse(
        "Elenco RolePerUser.xlsx",
        body,
        response,
  ...
  /**
   * Returns a ResponseEntity containing a ViewRolePerUserDto mapped by the Optional<RolePerUser>,
   * header and status passed in input
   */
  private ResponseEntity<ViewRolePerUserDto> toResponseEntity(
      Optional<RolePerUser> maybeResponse, HttpHeaders header, HttpStatus status) {
    return maybeResponse
        .map(response -> new ResponseEntity<>(rolePerUserMapper.map(response), header, status))
