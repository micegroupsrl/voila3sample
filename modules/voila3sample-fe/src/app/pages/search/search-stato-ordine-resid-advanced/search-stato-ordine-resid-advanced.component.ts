...
@Component({
    selector: 'app-search-stato-ordine-resid-advanced',
    templateUrl: './search-stato-ordine-resid-advanced.component.html',
    styleUrls: ['./search-stato-ordine-resid-advanced.component.scss']
})
export class SearchStatoOrdineResidAdvancedComponent extends BaseSearchResidAdvancedComponent implements OnInit {
    override attributeList = [
        // Definition of the object's list that will be used for build the filter
        { name: 'idStatoOrdine', type: 'number', api: ['minoreDi', 'uguale', 'maggioreDi'] },
        { name: 'descrizione', type: 'string', api: ['contiene'] }
    ];

    constructor(
        public statoOrdineDialogRef: MatDialogRef<SearchStatoOrdineResidAdvancedComponent>,
        private statoOrdineGroupApiService: StatoOrdineGroupApiService,
        private statoOrdineFb: FormBuilder,
        private changeDetector: ChangeDetectorRef,
        public override dialog: MatDialog,
        @Inject(MAT_DIALOG_DATA) public override data: any
    ) {
        super(statoOrdineDialogRef, statoOrdineFb, changeDetector, dialog, data);
    }
    ...
    public getFilter(): any {
        const searchStatoOrdine = this.searchForm.value;
