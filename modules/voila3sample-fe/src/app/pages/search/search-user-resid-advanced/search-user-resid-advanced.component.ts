...
@Component({
    selector: 'app-search-user-resid-advanced',
    templateUrl: './search-user-resid-advanced.component.html',
    styleUrls: ['./search-user-resid-advanced.component.scss']
})
export class SearchUserResidAdvancedComponent extends BaseSearchResidAdvancedComponent implements OnInit {
    override attributeList = [
        // Definition of the object's list that will be used for build the filter
        { name: 'userId', type: 'number', api: ['minoreDi', 'uguale', 'maggioreDi'] },
        { name: 'email', type: 'string', api: ['contiene'] },
        { name: 'password', type: 'string', api: ['contiene'] },
        { name: 'username', type: 'string', api: ['contiene'] }
    ];

    constructor(
        public userDialogRef: MatDialogRef<SearchUserResidAdvancedComponent>,
        ...
        super(userDialogRef, userFb, changeDetector, dialog, data);
    }
    ...
    public getFilter(): any {
        const searchUser = this.searchForm.value;
        if (!searchUser) {
            return null;
        }
