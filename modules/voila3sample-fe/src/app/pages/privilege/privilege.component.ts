...
import { SearchPrivilegeResidAdvancedComponent } from '../search/search-privilege-resid-advanced/search-privilege-resid-advanced.component';
import { PageObject } from 'src/app/shared/page-object.interface';
import { setOptions } from 'src/app/utilities/function/helper';
import { PRIVILEGE } from '../costants/privilege.constant';
import { Router } from '@angular/router';
import { PrivilegeApiService } from '../services/services-privilege/privilege-api.service';
...
export class PrivilegeComponent implements OnInit {
    ...
    loadData(pageObject: PageObject) {
        ...
        this.privilegeGroupApiService.privilege.getPrivilegeByCriteria(options).subscribe({
            next: (data: any) => {
    ...
    public printXlsxReport(): void {
        this.privilegeApiService.printXlsxReport();
    }
    ...
    openDialogResidAdvanced() {
        let dialogRef = this.dialog.open(SearchPrivilegeResidAdvancedComponent, {
            data: this.savedData,
            autoFocus: false,
            minWidth: '50vw'
        });
    ...
    public navigateToView(id: number): void {
        ...
        let pathDetailView = PRIVILEGE.PATH.DETAIL + id + PATH_VIEW;
    ...
    public getFilter(): any {
        ...
        if (searchPrivilege) {
            filterBuild = filterBuild.andLike('name', searchPrivilege.name);
        }
     ...
     */
    public onUp(): any {
        this.dataForm = this.searchPrivilegeForm.value;
