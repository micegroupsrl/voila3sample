...
import { SearchUserResidComponent } from '../search/search-user-resid/search-user-resid.component';
import { SearchUserResidAdvancedComponent } from '../search/search-user-resid-advanced/search-user-resid-advanced.component';
import { PageObject } from 'src/app/shared/page-object.interface';
import { setOptions } from 'src/app/utilities/function/helper';
import { USER } from '../costants/user.constant';
import { Router } from '@angular/router';
import { UserApiService } from '../services/services-user/user-api.service';
...
export class UserComponent implements OnInit {
    ...
    loadData(pageObject: PageObject) {
        ...
        this.userGroupApiService.user.getUserByCriteria(options).subscribe({
            next: (data: any) => {
    ...
    public printXlsxReport(): void {
        this.userApiService.printXlsxReport();
    }
     ...
     */
    openDialogResid() {
        let dialogRef = this.dialog.open(SearchUserResidComponent, {
            data: this.dataForm,
            autoFocus: false,
            minWidth: '50vw'
        });
    ...
    openDialogResidAdvanced() {
        let dialogRef = this.dialog.open(SearchUserResidAdvancedComponent, {
            data: this.savedData,
            autoFocus: false,
            minWidth: '50vw'
        });
    ...
    public navigateToView(id: number): void {
        ...
        let pathDetailView = USER.PATH.DETAIL + id + PATH_VIEW;
    ...
    public getFilter(): any {
        ...
        if (searchUser) {
            filterBuild = filterBuild.andLike('email', searchUser.email);
        }
     ...
     */
    public onUp(): any {
        this.dataForm = this.searchUserForm.value;
